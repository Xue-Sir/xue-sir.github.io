...
EdgeMd 是我开发的一款 Windows 桌面工具，面向本地 Markdown 文件提供侧边查看和轻量编辑能力。
...
我在开发和测试阶段也使用过 Obsidian 管理 Markdown 文档。Obsidian 可以承担长篇编辑、目录整理和链接管理，EdgeMd 负责将其中一份计划固定显示在桌面侧边。使用 VS Code、Typora、记事本或其他编辑器时，文件读写流程保持一致。
...
后续开发会继续围绕本地文件模型、侧边窗口交互和 Markdown 任务场景展开。
